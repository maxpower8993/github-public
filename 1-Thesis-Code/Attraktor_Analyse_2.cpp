...
#define PLUS 0; // entsprechend dem Lauf ändern

void update(int[][ADSIZE],int[],int[],int,int);
void vergleich(int[],int[],int,int*);
//
// Das Programm verfolgt fuer Seeds zu Fitness 0 die Adjazenzmatrizen und bestimmt die
// Attraktoren. Im Augenblick werden die Bilder jeweils 3mal (=bildwiederholung) durchlaufen und die
// neuronalen Aktivitäten immer zu Beginn randomisiert.
//
int main() {
  ofstream outfile( "./Test.txt", ios_base::out);
  //
  int matrixzaehler = 1; // Matrixzähler für die Ausgabe der Adjazenzmatrizen in Python-Code (numpy)
    int inhibitory=2;  // Anzahl inhibitorischer, interner Neuronen
  ...
  int iseed;          // Seed fuer den Zufallszahlgenerator
    int Seed[26]={350197,543154,553567,611900,654744,656496,742361,742797,893720,935882,972944,977089,1014952,1190451,1253115,1257940,1347490,1389907,1391065,1432409,1454022,1597839,1602477,1891594,1990965,1999287}; //2mio Plus0 Seeds
    int Anzahl_Seeds=sizeof(Seed)/sizeof(Seed[0]);  // Länge des Seed Arrays
  int probability=50; // Besetzungswahrscheinlichkeit der Linien in der Ad-Matrix
    ...
    int Zwei[10]={1,2,4,8,16,32,64,128,256,512};  // Potenzen von 2 für Zustandsangaben (MAXFEUER = 1)
    int Basins[Zwei[ADSIZE-3] + 1];    // (Ebenfalls nur für MAXFEUER = 1)
    ...
    //
  int threshold=1;   // Ende der Respons, ab hier ist die Aktivität MAXFEUER.
...
// cout << "maximale Seed " << SEED_MAX << "\n";
  ...
  //
  outfile << "#Programm: Evolution  Datum: xx.xx.xxxx\n\n";
 ...
 Lcounter=0;
  ...
  //
    counter=0;
  //
    for(iseed=0;iseed<Anzahl_Seeds;++iseed){ // Seeds durchlaufen die Liste in Seed[]
        srand(Seed[iseed]);                   // Initialisierung des Zufallszahlgenerators
//        cout << "---------------------------------------------------------------------------------------------------\n\n";
        cout << Seed[iseed] << "\n";
        outfile << "---------------------------------------------------------------------------------------------------\n\n";
        outfile << "Seed: " << Seed[iseed];      // << "\n";
        //
...
//                            in=in+1;
            ...
            for(i_att=1;i_att<nattractor;++i_att){
                ...
                if(ih3!=0){testparameter=1;
//                    cout << "TP=1    Seed: " << iseed << "   Bild: " << ibild << " ih1,ih2: " << ih1 << " , " << ih2 << "\n";
                    outfile << "TP=1    Seed: " << iseed << "   Bild: " << ibild << "\n";
                }  //  falls der Fehler>0 ist, wird testparameter=1 gesetzt
...
//             cout << "   Generation " << ngesamt << "  (Seed: " << iseed <<"  Fitness: " << fitness << ")\n";
//                 cout << "Bild: " << ibild <<  "   Anzahl der Basins: " << (nattractor-1) << "\n";
              outfile << "Bild: " << ibild <<  "   Anzahl der Basins: " << (nattractor-1) << "\n";
            for(ik=1;ik<nattractor;++ik){
//                 cout << "  Attraktor: " << ik;
//                cout << "  Basinsgroesse: " << Attraktor[ik][50];
//                cout << "  Anzahl Zustaende: " << Attraktor[ik][51] << "\n";
//                cout << "Werte: \n";
                 outfile << "  Attraktor: " << ik;
                 outfile << "  Basinsgroesse: " << Attraktor[ik][50];
                 outfile << "  Anzahl Zustaende: " << Attraktor[ik][51] << "\n";
                 outfile << "Werte: \n";
                 for(il=1;il<(Attraktor[ik][51]+1);++il){
//                    cout << il << "  " << Attraktor[ik][il] << " neuronale Aktivität: ";
                    outfile << il << "  " << Attraktor[ik][il] << " neuronale Aktivität: ";
                    ih1=Attraktor[ik][il];
                    for(in=3;in<ADSIZE;++in){
                        neuronen[in]=ih1%2;
                        ih1=(ih1-neuronen[in])/2;}
                    for(in=0;in<ADSIZE;++in){
//                        cout << neuronen[in] << " ";
                        outfile << neuronen[in] << " ";}
//                     cout << "\n";
                    outfile << "\n";
                }  // for il=1, il<Attraktor...
//                 count_AttStates=count_AttStates+Attraktor[ik][51];
//                Att_Prob=float(Attraktor[ik][50])/64.0;
            }    // for ik=1 , ik<nattractor
             outfile << "\n";
 //           cout << "\n";
//             count_Attractor=count_Attractor+(nattractor-1);
//             count_AverageError=0;
             ibild=ibild+1;
         }  //     while(ibild<4 ...
            if(testparameter==0){
                cout << "TP=0   Seed: " << Seed[iseed] << "\n";
            }
        //
//        cout << "\n";
        outfile << "\n";
        ...
        outfile << "====ADJAZENZMATRIX====" << "\n";
        outfile << "matrix" << matrixzaehler <<" = np.array([ \n";
        for(int i=0; i<ADSIZE; i++) {
          outfile << "[";
          for(int j=0; j<ADSIZE; j++) {
              cout <<" "<<adjazenz[i][j]<<" ";
              outfile <<" " << adjazenz[i][j] <<", ";
              }
          cout << "\n";
          outfile << "],\n";
        }
